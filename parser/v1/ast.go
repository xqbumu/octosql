...
func (node *With) Format(buf *TrackedBuffer) {
	buf.Myprintf("WITH %v %v", node.CommonTableExprs, node.Select)
}

func (node *With) walkSubtree(visit Visit) error {
	if err := Walk(visit, node.CommonTableExprs); err != nil {
		return err
	}
...
const (
	...
	NotLikeStr                      = "not like"
	LikeRegexpStr                   = "~"
	LikeRegexpCaseInsensitiveStr    = "~*"
	NotLikeRegexpStr                = "!~"
	NotLikeRegexpCaseInsensitiveStr = "!~*"
	RegexpStr                       = "regexp"
